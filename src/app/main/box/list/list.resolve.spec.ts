...
describe('ListResolve', () => {
  ...
  it('should call http', inject([ListResolve], (resolve: ListResolve) => {
    ...
    expect(mockAuthHttp.get).toHaveBeenCalledWith('boxes');
  }));

  it('should catch error and navigate', inject([ListResolve], (resolve: ListResolve) => {
    mockAuthHttp.get.and.returnValue(Observable.throw('test boxes list resolve error'));
    resolve.resolve().subscribe();
    expect(mockSpinner.incrementCounter).toHaveBeenCalledTimes(1);
    expect(mockSpinner.decrementCounter).toHaveBeenCalledTimes(1);
